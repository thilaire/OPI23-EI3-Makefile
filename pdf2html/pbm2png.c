...
make_page(int index)
{
 ...
 sprintf((char *)string,"%s%d.html",basename,index);
 f=fopen((char*)string,"w");
...
main(int argc, char **argv)
{
        ...
        dpi=atol(argv[1]);
        ppm=dpi*1000/25.4;
        basename=(unsigned char*)argv[2];
        titlestring=(unsigned char*)argv[3];
	bottom=(unsigned char*)argv[4];
	...
	if (argc>=9&&*argv[8]!='0'){
        ...
        sprintf((char*)string,"%s%d.png",basename,filenumber);
        filenumber++;
        of=fopen((char*)string,"w");
        open_png();
        for (z=oy;z;z--){
                if (!(z&15)){
                        fprintf(stderr,".");
